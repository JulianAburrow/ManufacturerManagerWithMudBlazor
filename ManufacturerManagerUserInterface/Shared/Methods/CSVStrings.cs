...
public static class CSVStrings
{
    public static string CreateWidgetsCSVString(List<WidgetModel> widgets)
    {
        ...
        widgetsSB.Append("Name,Manufacturer,Colour,Cost Price,Retail Price,Stock Level,Net Stock Value,Status");
        ...
        foreach (var widget in widgets)
        {
            ...
            widgetsSB.Append($"{RemoveCommas(widget.Name)},{RemoveCommas(widget.Manufacturer.Name)},{widget.Colour?.Name ?? "None"},{widget.CostPrice},{widget.RetailPrice},{widget.StockLevel.ToString()},{widgetStockValue},{widget.Status.StatusName}");
            widgetsSB.Append(Environment.NewLine);
        }
